...
def _find_peaks_reference(hist: np.ndarray, smoothing_window: int) -> np.ndarray:
    """Original peak-finding logic, kept as a fallback for unusual inputs."""
...
def find_peaks(hist: np.ndarray, smoothing_window: int) -> np.ndarray:
    ...
    if smoothing_window < 0 or not np.issubdtype(hist.dtype, np.floating) or not np.all(np.isfinite(hist)):
        return _find_peaks_reference(hist, smoothing_window)

    maxima = np.ones(hist.size, dtype=bool)
    maxima[1:] &= hist[1:] >= hist[:-1]
    maxima[:-1] &= hist[:-1] >= hist[1:]
    maxima[0] &= hist[0] >= hist[-1]
    maxima[-1] &= hist[-1] >= hist[0]
    peak_candidates = np.flatnonzero(maxima)

    if len(peak_candidates) == 0:
        return peak_candidates

    if smoothing_window > 0:
        padded = np.pad(hist, smoothing_window, mode='constant', constant_values=-np.inf)
        windows = sliding_window_view(padded, (2 * smoothing_window) + 1)
        candidate_windows = windows[peak_candidates]
        valid_peaks = peak_candidates[np.argmax(candidate_windows, axis=1) == smoothing_window]

    else:
        valid_peaks = peak_candidates

    if len(valid_peaks) > 2:
        valid_peaks = np.asarray(valid_peaks, dtype=int)
        peak_heights = hist[valid_peaks]
        interval_mins = np.minimum.reduceat(hist, valid_peaks)[: valid_peaks.size - 1]
        highest_mins = np.empty(valid_peaks.size, dtype=float)
        highest_mins[0] = interval_mins[0]
        highest_mins[-1] = interval_mins[-1]
        if valid_peaks.size > 2:
            highest_mins[1:-1] = np.maximum(interval_mins[:-1], interval_mins[1:])

        prominence = np.zeros(valid_peaks.size, dtype=float)
        valid_prominence = (peak_heights > 0) & (peak_heights > highest_mins)
        prominence[valid_prominence] = (
            (peak_heights[valid_prominence] - highest_mins[valid_prominence])
            / peak_heights[valid_prominence]
        )
        prominent_peaks = valid_peaks[prominence >= 0.1]
        if prominent_peaks.size == 0:
            return np.array([])
        return prominent_peaks

    return np.asarray(valid_peaks, dtype=int)
...
def apply_spline_smoothing(values: np.ndarray, smoothing_factor: float, n_bins: int = None) -> np.ndarray:
    ...
    # Scale smoothing factor based on number of bins, retaining the original
    # bounded range so the effective spline strength is comparable across
    # files with different numbers of time bins.
